...
public class EmpService {
    ...
    public ListDto<EmpDto> findAllByEcompidOrderByEname(EmpDto empDto) { /*SearchingDto searchingDto*/
        List<EmpInfo> EmpList = iempJpaTryRepository.findAllByEcompidOrderByEname(empDto.getSearchCompid());/*searchingDto*/
        int count = 0;
        int q = 0;
    ...
    public ListDto<EmpDto> emplistConditionSearch(EmpDto empDto) {
        ...
        if (empDto.getCondition().equals("ename")) {
            EmpList = iempJpaTryRepository.findAllByEname(empDto.getItem(), empDto.getSearchCompid());
        } else if (empDto.getCondition().equals("eemail")) {
            EmpList = iempJpaTryRepository.findAllByEemail(empDto.getItem(), empDto.getSearchCompid());
        } else if (empDto.getCondition().equals("ephone")) {
            EmpList = iempJpaTryRepository.findAllByEphone(empDto.getItem(), empDto.getSearchCompid());
        } else if (empDto.getCondition().equals("eposition")) {
            EmpList = iempJpaTryRepository.findAllByEposition(empDto.getItem(), empDto.getSearchCompid());
        } else if (empDto.getCondition().equals("eaffiliation")) {
            EmpList = iempJpaTryRepository.findAllByEaffiliation(empDto.getItem(), empDto.getSearchCompid());
        }
        List<EmpDto> EmpListData = QsolModelMapper.map(EmpList, EmpDto.class);
        return ListDto.<EmpDto>builder()
                .list(EmpListData)
